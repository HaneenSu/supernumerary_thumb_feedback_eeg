...
% latencies are shifted by +1000 samples to correct for a 1-second
% hardware delay between the trigger and actual vibration onset.
%
% Output epochs (.set files) are saved to ../derivatives/epoched/VF/.
%
% Expected folder layout (relative to code/):
%   ../rawdata/SFV/              — raw VF EEG data (.vhdr files)
%   ../derivatives/epoched/VF/  — epoched output
%
...
for sub = 1:nSubject
    ...
    set_file = [path_to_rawdata 'SFV' four_digit_text '.vhdr'];
    ...
    % Adjust latency: VT protocol had a 1s hardware delay between trigger and
    % actual vibration onset, so shift all event latencies by +1000 samples
    for j = 1:size(subEEG.event, 2)
        subEEG.event(j).latency = subEEG.event(j).latency + 1000;
    end
